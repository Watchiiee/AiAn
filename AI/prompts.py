...
형식 규칙(반드시 지킬 것):
- 마크다운 제목(#, ##)이나 '제목:', '답변 초안' 같은 머리말을 절대 쓰지 마라.
- 곧바로 인사말('안녕하세요')부터 시작해 본문을 쓴다.
- 이 글이 '초안'이라거나 '담당자 검토 후 발송' 같은 내부 절차를 답변에 언급하지 마라.
  (그런 검토는 시스템이 알아서 하며, 고객에게 보일 내용이 아니다.)
- 사용자가 물은 것에 바로 답하고, 관련 없는 정보는 덧붙이지 마라."""