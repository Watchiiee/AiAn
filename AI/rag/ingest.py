...
도메인별로 별도 컬렉션을 만든다 (라우팅용):
    data/knowledge_base/admin/     → 컬렉션 minwon_admin
    data/knowledge_base/technical/ → 컬렉션 minwon_technical

실행 (레포 루트에서): python -m AI.rag.ingest
"""
import os
import glob
import chromadb
...
def parse_markdown(path: str) -> list[dict]:
    ...
    def flush():
        ...
        for piece in _split_long(full):
            chunks.append({
                "text": piece, "source": source, "heading": cur_heading, "section": parent_h2,
            })
...
def _ingest_domain(client, domain: str, collection_name: str) -> int:
    ...
    collection.add(
        ...
        documents=texts,
        metadatas=[
            {"source": c["source"], "heading": c["heading"], "section": c["section"], "domain": domain}
            for c in all_chunks
        ],
    )
