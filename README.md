...
전체 의사결정 과정과 실패·재현 사례는 `DECISION_LOG.md`에 A~AA절로 상세히
기록되어 있습니다.
...
├── DECISION_LOG.md                 # 전체 의사결정 기록 (A~AA절)
...
(쿨다운 300초 — 서버가 안정화될 시간을 벌기 위함). 임계값은 "위험해진
뒤"가 아니라 "새 서버가 준비되는 시간(수십 초~1분)을 감안해 미리
대응"하는 것이 목적이라, 통상적으로 쓰이는 60~70% 선을 채택했습니다
(95% 등 더 높은 값은 자동 확장 트리거보다 사람에게 보내는 긴급 알림
임계값으로 더 적합).
...
"수정"으로는 바뀌지 않는다는 것을 뒤늦게 발견했습니다(VPC/Subnet,
Launch Configuration에서 이미 겪었던 "생성 시점에만 고정되는 설정" 패턴이
여기도 있었음). 결국 ASG 그룹 자체를 새로 만들고, 그 안에서 Scaling
정책 2개를 재생성하고, 기존 Cloud Insight 알람의 "액션"만 새 그룹의
새 정책으로 다시 연결하는 방식으로 해결했습니다.
